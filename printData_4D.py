...
X = np.zeros((csvreader.line_num-1,3))
...
# img = ax.scatter(X[0], X[1], y, c=X[2], cmap=plt.hot())
# fig.colorbar(img)
ax.set_xlabel('Last time step [s]')
ax.set_ylabel('AvHS volume above 880 °C [%]')
ax.set_zlabel('Relative Density [%]');
# ax.set_title('Visualisation Dataset')
plt.savefig("./Visual/VisualisationDataset_LTS_AvHS.eps", format='eps',bbox_inches='tight') # saved as eps for high quality pictures
plt.savefig("./Visual/VisualisationDataset_LTS_AvHS.svg", format='svg',bbox_inches='tight')

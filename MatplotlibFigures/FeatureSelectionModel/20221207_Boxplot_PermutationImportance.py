...
for k in range(100):
    ...
    print(k)
    idbm=pd.read_csv("D:/Benutzerdateien/OneDrive - tuhh.de/TUHH/Semester 9/Studienarbeit/Python/Maschine Learning/Results/ModelTraining/FeatureSelectionAlgorithm/PermutationImportance/Matrix/{}.csv".format(k), encoding='latin1')
    
    LasPowTemp=idbm['Laser Power [W]'].tolist()
    ScaSpeTemp=idbm['Scan Speed [mm/s]'].tolist()
    HatDisTemp=idbm['Hatch Distance [mm]'].tolist()
    DurTemp=idbm['Duration [s]'].tolist()
    AvIntTemp=idbm['Average Interlayertemperature [Â°C]'].tolist()

    LasPow.append(LasPowTemp[0])
    LasPowstd.append(LasPowTemp[1])

    ScaSpe.append(ScaSpeTemp[0])
    ScaSpestd.append(ScaSpeTemp[1])
    
    HatDis.append(HatDisTemp[0])
    HatDisstd.append(HatDisTemp[1])

    Dur.append(DurTemp[0])
    Durstd.append(DurTemp[1])

    AvInt.append(AvIntTemp[1])
    AvIntstd.append(AvIntTemp[0])
    
dataAver = [LasPow,ScaSpe,HatDis,AvInt,Dur]
datastd = [LasPowstd,ScaSpestd,HatDisstd,AvIntstd,Durstd]
...
LasPowav = Average(LasPow)
ScaSpeav = Average(ScaSpe)
HatDisav = Average(HatDis)
...
# ax1.set_ylim(0,13.5)
plt.plot([1],[LasPowav], marker='x', color='black')
plt.plot([2],[ScaSpeav], marker='x', color='black')
plt.plot([3],[HatDisav], marker='x', color='black')
plt.plot([4],[AvIntav], marker='x', color='black')
plt.plot([5],[Durav], marker='x', color='black')
...
plt.savefig("D:/Benutzerdateien/OneDrive - tuhh.de/TUHH/Semester 9/Studienarbeit/LateX/08_validation/fig/FeatureSelectionAlgorithm/PermutationImportance/Boxplot.pdf", format='pdf',bbox_inches='tight') # saved as eps for high quality pictures
